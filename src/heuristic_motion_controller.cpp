...
#include <franka_valve/heuristic_motion_controller.h>
...
bool HeuristicMotionController::init(hardware_interface::RobotHW* robot_hw,
                                     ros::NodeHandle& node_handle) {
  string arm_id;
  if (!node_handle.getParam("arm_id", arm_id)) {
    ROS_ERROR("HeuristicMotionController: Could not read parameter arm_id");
    return false;
  }
  vector<string> joint_names;
  if (!node_handle.getParam("joint_names", joint_names) || joint_names.size() != 7) {
    ROS_ERROR(
        "HeuristicMotionController: Invalid or no joint_names parameters provided, aborting "
        "controller init!");
    return false;
  }
  ...
  if (!node_handle.getParam("k_gains", k_gains_) || k_gains_.size() != 7) {
    ROS_ERROR(
        "HeuristicMotionController:  Invalid or no k_gain parameters provided, aborting "
        "controller init!");
    return false;
  } else {
  ...
  if (!node_handle.getParam("d_gains", d_gains_) || d_gains_.size() != 7) {
    ROS_ERROR(
        "HeuristicMotionController:  Invalid or no d_gain parameters provided, aborting "
        "controller init!");
    return false;
  } else {
  ...
  if (!node_handle.getParam("xk_gains", xk_gains_) || xk_gains_.size() != 6) {
    ROS_ERROR(
        "HeuristicMotionController:  Invalid or no xk_gain parameters provided, aborting "
        "controller init!");
    return false;
  } else {
  ...
  if (!node_handle.getParam("xd_gains", xd_gains_) || xd_gains_.size() != 6) {
    ROS_ERROR(
        "HeuristicMotionController:  Invalid or no xd_gain parameters provided, aborting "
        "controller init!");
    return false;
  } else {
  ...
  if (!node_handle.getParam("coriolis_factor", coriolis_factor_)) {
    ROS_INFO_STREAM("HeuristicMotionController: coriolis_factor not found. Defaulting to "
                    << coriolis_factor_);
  }

  auto* state_interface = robot_hw->get<franka_hw::FrankaStateInterface>();
  if (state_interface == nullptr) {
    ROS_ERROR_STREAM("HeuristicMotionController: Error getting state interface from hardware");
    return false;
  }
  try {
    state_handle_ =
    ...
    ROS_ERROR_STREAM(
        "HeuristicMotionController: Exception getting state handle from interface: " << ex.what());
    return false;
  }

  auto* model_interface = robot_hw->get<franka_hw::FrankaModelInterface>();
  if (model_interface == nullptr) {
    ROS_ERROR_STREAM("HeuristicMotionController: Error getting model interface from hardware");
    return false;
  }
  try {
    model_handle_ =
    ...
    ROS_ERROR_STREAM(
        "HeuristicMotionController: Exception getting model handle from interface: " << ex.what());
    return false;
  }
  ...
  if (effort_joint_interface == nullptr) {
    ROS_ERROR_STREAM(
        "HeuristicMotionController: Error getting effort joint interface from hardware");
    return false;
  }
  for (size_t i = 0; i < 7; ++i) {
    try {
      ...
      ROS_ERROR_STREAM("HeuristicMotionController: Exception getting joint handles: " << ex.what());
      return false;
    }
  }
  return true;
...
void HeuristicMotionController::starting(const ros::Time& /* time */) {
...
void HeuristicMotionController::update(const ros::Time& /*time*/, const ros::Duration& period) {
...
array<double, 7> HeuristicMotionController::saturateTorqueRate(
...
void HeuristicMotionController::InitMotionPlan(double init_theta, double goal_theta) {
...
void HeuristicMotionController::UpdateStates() {
  const franka::RobotState& robot_state_ = state_handle_->getRobotState();
  array<double, 7> coriolis_array = model_handle_->getCoriolis();
...
array<double, 7> HeuristicMotionController::OperationalSpaceControl() {
  const franka::RobotState& robot_state_ = state_handle_->getRobotState();
...
// void HeuristicMotionController::CLIK()
// {
...
array<double, 7> HeuristicMotionController::JointControl() {
  const franka::RobotState& robot_state_ = state_handle_->getRobotState();
...
HeuristicMotionController::Target HeuristicMotionController::TargetTransformMatrix(Objects obj,
                                                                                   Robot robot,
                                                                                   double angle) {
  // frame u : universal
  ...
  Tge << cos(robot.ee_align), -sin(robot.ee_align), 0, sin(robot.ee_align), cos(robot.ee_align), 0,
      0, 0, 1;
  Tug = HeuristicMotionController::R3D(obj, -obj.o_margin.normalized(), angle);
  Tue << Tug * Tge;
...
Matrix3d HeuristicMotionController::R3D(Objects obj, Vector3d unitVec, double angle) {
  Matrix3d Tug;
...
PLUGINLIB_EXPORT_CLASS(franka_valve::HeuristicMotionController,
