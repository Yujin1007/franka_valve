...
#include <franka_valve/trained_controller.h>
...
bool TrainedController::init(hardware_interface::RobotHW* robot_hw, ros::NodeHandle& node_handle) {
  string arm_id;
  if (!node_handle.getParam("arm_id", arm_id)) {
    ROS_ERROR("TrainedController: Could not read parameter arm_id");
    return false;
  }
  vector<string> joint_names;
  if (!node_handle.getParam("joint_names", joint_names) || joint_names.size() != 7) {
    ROS_ERROR(
        "TrainedController: Invalid or no joint_names parameters provided, aborting "
        "controller init!");
    return false;
  }
  ...
  if (!node_handle.getParam("k_gains", k_gains_) || k_gains_.size() != 7) {
    ROS_ERROR(
        "TrainedController:  Invalid or no k_gain parameters provided, aborting "
        "controller init!");
    return false;
  } else {
  ...
  if (!node_handle.getParam("d_gains", d_gains_) || d_gains_.size() != 7) {
    ROS_ERROR(
        "TrainedController:  Invalid or no d_gain parameters provided, aborting "
        "controller init!");
    return false;
  } else {
  ...
  if (!node_handle.getParam("xk_gains", xk_gains_) || xk_gains_.size() != 6) {
    ROS_ERROR(
        "TrainedController:  Invalid or no xk_gain parameters provided, aborting "
        "controller init!");
    return false;
  } else {
  ...
  if (!node_handle.getParam("xd_gains", xd_gains_) || xd_gains_.size() != 6) {
    ROS_ERROR(
        "TrainedController:  Invalid or no xd_gain parameters provided, aborting "
        "controller init!");
    return false;
  } else {
  ...
  if (!node_handle.getParam("coriolis_factor", coriolis_factor_)) {
    ROS_INFO_STREAM("TrainedController: coriolis_factor not found. Defaulting to " << coriolis_factor_);
  }

  auto* state_interface = robot_hw->get<franka_hw::FrankaStateInterface>();
  if (state_interface == nullptr) {
    ROS_ERROR_STREAM("TrainedController: Error getting state interface from hardware");
    return false;
  }
  try {
    state_handle_ =
    ...
    ROS_ERROR_STREAM("TrainedController: Exception getting state handle from interface: " << ex.what());
    return false;
  }

  auto* model_interface = robot_hw->get<franka_hw::FrankaModelInterface>();
  if (model_interface == nullptr) {
    ROS_ERROR_STREAM("TrainedController: Error getting model interface from hardware");
    return false;
  }
  try {
    model_handle_ =
    ...
    ROS_ERROR_STREAM("TrainedController: Exception getting model handle from interface: " << ex.what());
    return false;
  }

  auto* effort_joint_interface = robot_hw->get<hardware_interface::EffortJointInterface>();
  if (effort_joint_interface == nullptr) {
    ROS_ERROR_STREAM("TrainedController: Error getting effort joint interface from hardware");
    return false;
  }
  for (size_t i = 0; i < 7; ++i) {
    try {
      ...
      ROS_ERROR_STREAM("TrainedController: Exception getting joint handles: " << ex.what());
      return false;
    }
  }
  return true;
...
void TrainedController::starting(const ros::Time& /* time */) {
  ...
  _x_plot.setZero(1000, 6);
  _xdes_plot.setZero(1000, 6);
  ...
  _obj.grab_dir << _obj.r_margin;  // obj.o_margin.cross(obj.r_margin);
  _obj.pos << 0.43, 0.0, 0.9;
  ...
  _obs_obj << _obj.pos(0), _obj.pos(1), _obj.pos(2), rot_obj(0, 0), rot_obj(0, 1), rot_obj(0, 2),
      rot_obj(0, 1), rot_obj(1, 1), rot_obj(1, 2), _init_theta, direction, 1, 0, 1;
  UpdateStates();
}

void TrainedController::update(const ros::Time& /*time*/, const ros::Duration& period) {
  ...
  if (!_motion_done) {
    ...
    if (_cnt_plot != 0) {
      printf("save csv\n");
      std::string filename = "joint_gain.csv";
      writeToCSVfile(filename, _qdes_plot, _q_plot, _xdes_plot, _x_plot);
...
void TrainedController::writeToCSVfile(string filename,
                                  MatrixXd qd,
                                  MatrixXd q,
                                  MatrixXd xd,
                                  MatrixXd x) {
...
void TrainedController::gripperCloseCallback(const std_msgs::Bool& msg) {
  gripper_close = msg.data;
  gripper_open = false;
}

void TrainedController::gripperOpenCallback(const std_msgs::Bool& msg) {
  gripper_open = msg.data;
  gripper_close = false;
}

array<double, 7> TrainedController::saturateTorqueRate(
...
void TrainedController::InitMotionPlan(double init_theta, double goal_theta) {
...
void TrainedController::MotionPlan(Target target) {
...
void TrainedController::UpdateStates() {
  const franka::RobotState& robot_state_ = state_handle_->getRobotState();
  array<double, 7> coriolis_array = model_handle_->getCoriolis();
...
void TrainedController::Observation() {
  _obs_q.tail((_stack - 1) * 7) = _obs_q.head((_stack - 1) * 7);
  _obs_rpy.tail((_stack - 1) * 6) = _obs_q.head((_stack - 1) * 6);
  _obs_rpyplan.tail((_stack - 1) * 6) = _obs_q.head((_stack - 1) * 6);
  _obs_posplan.tail((_stack - 1) * 3) = _obs_q.head((_stack - 1) * 3);
  _obs_pos.tail((_stack - 1) * 3) = _obs_q.head((_stack - 1) * 3);

  _obs_q.head(7) << _q;
  _obs_rpy.head(6) << _R_hand(0, 0), _R_hand(0, 1), _R_hand(0, 2), _R_hand(0, 1), _R_hand(1, 1),
      _R_hand(1, 2);
  _obs_rpyplan.head(6) << _R_des_hand(0, 0), _R_des_hand(0, 1), _R_des_hand(0, 2),
      _R_des_hand(0, 1), _R_des_hand(1, 1), _R_des_hand(1, 2);
...
array<double, 7> TrainedController::OperationalSpaceControl() {
  const franka::RobotState& robot_state_ = state_handle_->getRobotState();
...
array<double, 7> TrainedController::JointControl() {
  const franka::RobotState& robot_state_ = state_handle_->getRobotState();
...
TrainedController::Target TrainedController::TargetTransformMatrix(Objects obj, Robot robot, double angle) {
  // frame u : universal
  ...
  Tge << cos(robot.ee_align), -sin(robot.ee_align), 0, sin(robot.ee_align), cos(robot.ee_align), 0,
      0, 0, 1;
  Tug = TrainedController::R3D(obj, -obj.o_margin.normalized(), angle);
  Tue << Tug * Tge;
...
Matrix3d TrainedController::R3D(Objects obj, Vector3d unitVec, double angle) {
  Matrix3d Tug;
...
PLUGINLIB_EXPORT_CLASS(franka_valve::TrainedController, controller_interface::ControllerBase)